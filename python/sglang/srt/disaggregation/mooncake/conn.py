...
class MooncakeKVManager(StagingManagerMixin, CommonKVManager):
    ...
    def send_kvcache_dcp(
        ...
        *,
        ...
        # Capacity batching is limited to target-only transfers. Keep the
        # upstream full-plan send (including pack fallback) for draft KV.
        if num_draft > 0:
            return self._send_kvcache_dcp_slice(
                mooncake_session_id,
                src_kv_ptrs,
                dst_kv_ptrs,
                plan,
        ...
        num_target_tokens = plan.target_src_token_indices.size
        if num_target_tokens == 0:
            return 0
        tokens_per_batch = num_target_tokens
        if pack_buffer is not None:
            bytes_per_token = sum(dcp_token_item_lens[:num_target])
        ...
        for start in range(0, num_target_tokens, tokens_per_batch):
            end = start + tokens_per_batch
            batch_plan = dataclasses.replace(
                plan,
                target_src_token_indices=plan.target_src_token_indices[start:end],
                target_dst_token_indices=plan.target_dst_token_indices[start:end],
            )
